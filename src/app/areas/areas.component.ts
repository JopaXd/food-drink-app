...
export class AreasComponent implements OnInit {
  ...
  countryData = [
    ...
    {"Code":"CN","Country":"China","ISO":51,"Nationality":"Chinese"},
  	...
  	this._areaSvc.getAllAreas().subscribe(data => {
  		...
  		countries.forEach((country:any) => {
  		   for (let i = 0; i < this.countryData.length; i++) {
  		     if (this.countryData[i]["Nationality"] === country["strArea"]){
  		     	this.countriesList.push({"country_code" : this.countryData[i]["Code"], "Nationality": country["strArea"]});
  		     	return;
  		     }
  		   } 
  		});
